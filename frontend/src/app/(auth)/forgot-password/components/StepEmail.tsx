...
export default function StepEmail({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18, animation: "lgFadeUp 0.4s ease both", opacity: 0 }}>
      <div>
        <label style={{ display: "flex", alignItems: "center", gap: 7, fontFamily: "'Inter',system-ui,sans-serif", fontSize: 13, fontWeight: 600, color: C.text, marginBottom: 7, letterSpacing: "-0.01em" }}>
      ...
      <button
        ...
        style={{
          ...
          boxShadow: loading || !email ? "none" : "0 4px 18px rgba(26,86,219,0.38)",
        }}
        onMouseEnter={(e) => { if (!loading && email) { (e.currentTarget as HTMLElement).style.background = C.blueHover; (e.currentTarget as HTMLElement).style.transform = "translateY(-1px)"; }}}
        onMouseLeave={(e) => { if (!loading && email) { (e.currentTarget as HTMLElement).style.background = C.blue; (e.currentTarget as HTMLElement).style.transform = "translateY(0)"; }}}
      >
        {loading ? (
          <>
