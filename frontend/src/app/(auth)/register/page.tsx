...
function InputField({
  ...
  return (
    ...
    >
      <span style={{ color: focused ? C.blue : "#93C5FD", flexShrink: 0, display: "flex" }}>
        {icon}
      </span>
      <input
        type={type}
...
const IconAt = (
  <svg width="17" height="17" viewBox="0 0 24 24" fill="none">
    ...
    <path d="M16 8v5a3 3 0 006 0v-1a10 10 0 10-3.92 7.94" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
  </svg>
);
...
export default function RegisterPage() {
  ...
  const fields = [
    { label: "Full Name",        type: "text",     placeholder: "Enter your full name",     value: fullName,     onChange: setFullName,     icon: IconPerson },
    { label: "Username",         type: "text",     placeholder: "Enter your username",      value: username,     onChange: setUsername,     icon: IconAt     },
    { label: "Email",            type: "email",    placeholder: "Enter your email",         value: email,        onChange: setEmail,        icon: IconMail   },
    { label: "Create password",  type: "password", placeholder: "Enter your password",      value: password,     onChange: setPassword,     icon: IconLock   },
    { label: "Confirm password", type: "password", placeholder: "Confirm your password",    value: confirmPass,  onChange: setConfirmPass,  icon: IconLock   },
  ];
    ...
    >
      ...
      <div
        style={{
          position: "absolute",
          inset: 0,
          backgroundImage: "radial-gradient(rgba(26,86,219,0.25) 1px, transparent 1px)",
      ...
      >
        ...
        <div style={{ padding: "36px 40px 40px" }}>
          ...
          >
            ...
            <p style={{ fontFamily: "'Inter', system-ui, sans-serif", fontSize: 14, color: C.muted, margin: 0, lineHeight: 1.6 }}>
          ...
          >
            {fields.map(({ label, type, placeholder, value, onChange, icon }) => (
              <div key={label}>
                <label
                  style={{
                    display: "block",
                ...
                <InputField type={type} placeholder={placeholder} value={value} onChange={onChange} icon={icon} />
              </div>
            ))}
            ...
            <button
              ...
              onMouseEnter={(e) => {
                if (!loading) {
                  ...
                  (e.currentTarget as HTMLElement).style.boxShadow = "0 6px 22px rgba(26,86,219,0.46)";
                }
              }}
              onMouseLeave={(e) => {
                if (!loading) {
                  ...
                  (e.currentTarget as HTMLElement).style.boxShadow = "0 4px 18px rgba(26,86,219,0.38)";
                }
              }}
            >
              {loading ? (
            ...
            <p style={{ fontFamily: "'Inter', system-ui, sans-serif", fontSize: 13.5, color: C.muted, textAlign: "center", margin: 0, lineHeight: 1 }}>
              Already have an account?{" "}
              <Link
                href="/login"
                style={{ color: C.blue, fontWeight: 600, textDecoration: "none" }}
                onMouseEnter={(e) => ((e.currentTarget as HTMLElement).style.color = C.blueHover)}
        ...
        >
          <span style={{ fontFamily: "'Inter', system-ui, sans-serif", fontSize: 11, color: C.muted }}>
            © 2025 Geomarketia
          </span>
          <div style={{ display: "flex", gap: 14 }}>
            {["Privacy", "Terms"].map((t) => (
              <Link
                key={t}
                href={`/${t.toLowerCase()}`}
                style={{ fontFamily: "'Inter', system-ui, sans-serif", fontSize: 11, color: C.blue, textDecoration: "none", fontWeight: 500 }}
              >
                {t}
              </Link>
            ))}
