  ...
  export function QrisTab() {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "1rem" }}>
        <p style={{ margin: 0, fontSize: "0.92rem", fontWeight: 700, color: "#111827" }}>Scan QR Code to Pay</p>
        <QRCode />
        <p style={{ margin: 0, fontSize: "0.75rem", color: "#9CA3AF", textAlign: "center", maxWidth: 280 }}>Open your mobile banking or e-wallet app and scan the QR code above.</p>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, justifyContent: "center", width: "100%" }}>
          {wallets.map((w) => (
            <div key={w.name} style={{ padding: "4px 10px", borderRadius: 20, background: w.bg, border: `1px solid ${w.color}22`, fontSize: "0.72rem", fontWeight: 700, color: w.color }}>{w.name}</div>
          ))}
        </div>
