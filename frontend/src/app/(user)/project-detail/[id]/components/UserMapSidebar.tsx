...
function renderStars(rating: number) {
  ...
  for (let i = 0; i < 5; i++) {
    if (i < full) {
      ...
      stars.push(
        <svg key={i} width="11" height="11" viewBox="0 0 24 24" fill="#e2e8f0" stroke="#e2e8f0" strokeWidth="1">
          <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
        </svg>,
      );
    }
...
function CategoryIcon({ category }: { category: string }) {
  ...
  if (cat.includes("restoran") || cat.includes("makan") || cat.includes("food") || cat.includes("cafe") || cat.includes("kafe") || cat.includes("kuliner") || cat.includes("coffee")) {
    return (
  ...
  if (cat.includes("toko") || cat.includes("shop") || cat.includes("mall") || cat.includes("market") || cat.includes("belanja") || cat.includes("retail") || cat.includes("supermarket") || cat.includes("minimarket")) {
    return (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  if (cat.includes("hotel") || cat.includes("penginapan") || cat.includes("inn") || cat.includes("resort") || cat.includes("villa") || cat.includes("hostel")) {
    return (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  if (cat.includes("rumah sakit") || cat.includes("klinik") || cat.includes("apotek") || cat.includes("health") || cat.includes("medis") || cat.includes("dokter") || cat.includes("farmasi")) {
    return (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  if (cat.includes("sekolah") || cat.includes("kampus") || cat.includes("universitas") || cat.includes("pendidikan") || cat.includes("school") || cat.includes("edu")) {
    return (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  if (cat.includes("bank") || cat.includes("keuangan") || cat.includes("atm") || cat.includes("finance")) {
    return (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  if (cat.includes("salon") || cat.includes("spa") || cat.includes("kecantikan") || cat.includes("beauty") || cat.includes("barbershop")) {
    return (
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "8px" }}>
        {!clickedPoint && (
          ...
          >
            ...
            <div style={{ fontSize: "12px", marginTop: "4px", lineHeight: 1.6 }}>
          ...
          return (
            <div
              ...
              style={{
                ...
                border: isSelected ? "1.5px solid #1A56DB" : "1px solid #f1f5f9",
...
export function UserClusterSummaryCard({
  ...
  const clusterData = new Map<number, { count: number; kecCount: Map<string, number> }>();
  ...
  const rawCards = Array.from(clusterData.entries()).map((entry) => {
    ...
    const sortedKec = Array.from(kecCount.entries()).sort((a, b) => b[1] - a[1]);
    const kec1 = sortedKec[0]?.[0] ?? "";
    return { geoCluster, color, count, isNoise, kec1 };
  });

  const cards: CardEntry[] = rawCards.map(({ geoCluster, color, count, isNoise, kec1 }) => {
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <div>
            <div style={{ fontSize: "13px", fontWeight: 700, color: "#0f172a" }}>Area Summary</div>
        ...
        {cards.map(({ geoCluster, color, label, count, isNoise }) => {
          ...
          return (
            <div
              ...
              style={{
                background: isActive ? (isNoise ? "#f1f5f9" : "#EBF3FF") : isNoise ? "#f8fafc" : "#fafbff",
                borderRadius: "12px",
                border: isActive ? `2px solid ${color}` : `1px solid ${isNoise ? "#e2e8f0" : "#EBF3FF"}`,
