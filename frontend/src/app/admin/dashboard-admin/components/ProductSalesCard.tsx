  ...
  return (
    <div style={{ display: "flex", justifyContent: "center" }}>
      ...
      >
        ...
        {hov === null && (
          <>
            <text x={cx} y={cy - 10} textAnchor="middle" fontSize="14" fill="#94a3b8" fontFamily="'DM Sans', sans-serif" fontWeight={500}>
              Total Terjual
            </text>
            <text x={cx} y={cy + 16} textAnchor="middle" fontSize="32" fill="#0f172a" fontFamily="'DM Sans', sans-serif" fontWeight={800}>
              {totalSold}
            </text>
            <text x={cx} y={cy + 36} textAnchor="middle" fontSize="12" fill="#94a3b8" fontFamily="'DM Sans', sans-serif">
              produk
            </text>
          </>
        )}
        ...
        {hov !== null && (() => {
          ...
          return (
            <g pointerEvents="none">
              ...
              <text x={px + 27} y={py + 22} fontSize="11" fill="#374151" fontFamily="'DM Sans', sans-serif" fontWeight={600}>
                {hov.label}
              </text>
              <rect x={px + pw - 46} y={py + 10} width={38} height={16} rx={8} fill={hov.color + "22"} />
              <text x={px + pw - 27} y={py + 21} textAnchor="middle" fontSize="9.5" fill={hov.color} fontFamily="'DM Sans', sans-serif" fontWeight={700}>
                {(hov.pct * 100).toFixed(1)}%
              </text>
              <text x={px + 12} y={py + 44} fontSize="12" fill="#0f172a" fontFamily="'DM Sans', sans-serif" fontWeight={800}>
                {hov.sold} produk terjual
              </text>
            </g>
          );
        })()}
