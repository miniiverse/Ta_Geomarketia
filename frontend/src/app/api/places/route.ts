...
export async function GET(request: NextRequest) {
  ...
  if (!apiUrl) return NextResponse.json({ message: 'api_url required' }, { status: 400 });
  ...
  try {
    ...
    for (let i = 2; i <= totalPages; i += 5) {
      ...
      const batchRes = await Promise.all(
        batch.map((p) =>
          fetch(`${apiUrl}/places?page=${p}&page_size=50`, { cache: 'no-store' })
            .then((r) => r.json())
            .then((d) => d.data ?? [])
            .catch(() => [])
        )
    ...
    const valid = allData.filter(
      (p: any) =>
        p.latitude  >= 0.8  && p.latitude  <= 1.3 &&
        p.longitude >= 103.6 && p.longitude <= 104.4
    );
    ...
    return NextResponse.json({ message: err.message, data: [] }, { status: 500 });
  }
}